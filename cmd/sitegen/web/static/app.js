const translations={
zh:{catalog:"模型目录",docs:"项目文档",heroTitle:"在一个目录里了解主流 LLM",heroCopy:"从项目维护的权威 YAML 数据自动生成，快速比较上下文窗口、最大输出、多模态和工具调用等关键能力。",copy:"复制",copied:"已复制",explore:"探索模型",catalogTitle:"模型目录",searchLabel:"搜索模型",searchPlaceholder:"搜索模型、厂商、ID 或别名",allProviders:"所有厂商",sortRecommended:"最新与权威厂商优先",sortReleased:"按发布时间排序",sortAuthority:"按权威厂商排序",sortName:"按名称排序",sortContext:"上下文从大到小",sortOutput:"最大输出从大到小",clear:"清除筛选",emptyTitle:"没有匹配的模型",emptyCopy:"尝试清除部分筛选条件或使用其他关键词。",footer:"静态数据，随每次发布自动更新",models:"模型",providers:"厂商",multimodal:"多模态",toolUse:"工具调用",reasoning:"推理模型",results:"个模型",context:"上下文",output:"最大输出",released:"发布时间",unknown:"未公布",official:"官方页面",modelCard:"模型卡",documentation:"文档",paper:"论文",repository:"仓库",website:"厂商官网"},
en:{catalog:"Model catalog",docs:"Documentation",heroTitle:"Understand leading LLMs in one catalog",heroCopy:"Automatically generated from the project's authoritative YAML registry. Compare context windows, output limits, modalities and tool-use capabilities.",copy:"Copy",copied:"Copied",explore:"EXPLORE",catalogTitle:"Model catalog",searchLabel:"Search models",searchPlaceholder:"Search models, providers, IDs or aliases",allProviders:"All providers",sortRecommended:"Newest & leading providers",sortReleased:"Sort by release date",sortAuthority:"Sort by leading providers",sortName:"Sort by name",sortContext:"Largest context first",sortOutput:"Largest output first",clear:"Clear filters",emptyTitle:"No models found",emptyCopy:"Try removing a filter or using a different search term.",footer:"Static metadata, automatically updated with every release",models:"Models",providers:"Providers",multimodal:"Multimodal",toolUse:"Tool use",reasoning:"Reasoning",results:"models",context:"Context",output:"Max output",released:"Released",unknown:"Unpublished",official:"Official",modelCard:"Model card",documentation:"Docs",paper:"Paper",repository:"Repository",website:"Provider website"}
};
const tagLabels={chat:["对话","Chat"],embedding:["向量","Embedding"],rerank:["重排","Rerank"],tts:["语音合成","TTS"],asr:["语音识别","ASR"],"tool-use":["工具调用","Tool use"],"structured-output":["结构化输出","Structured output"],multimodal:["多模态","Multimodal"],vision:["图片输入","Vision"],"image-output":["图片生成","Image output"],"audio-input":["音频输入","Audio input"],"audio-output":["音频输出","Audio output"],"video-input":["视频输入","Video input"],"video-output":["视频输出","Video output"],"file-input":["文件输入","File input"],reasoning:["推理","Reasoning"],preview:["预览","Preview"],experimental:["实验性","Experimental"],fast:["快速","Fast"],mini:["Mini","Mini"],nano:["Nano","Nano"],turbo:["Turbo","Turbo"],pro:["Pro","Pro"],thinking:["思考","Thinking"]};
...
const state={catalog:null,lang:localStorage.getItem("museum-language")||((navigator.language||"").toLowerCase().startsWith("zh")?"zh":"en"),query:"",provider:"",sort:"recommended",tags:new Set()};
...
const byName=(a,b)=>a.name.localeCompare(b.name,state.lang==="zh"?"zh":"en");
const byRelease=(a,b)=>(b.released_at||0)-(a.released_at||0)||(b.authority||0)-(a.authority||0)||byName(a,b);
const byAuthority=(a,b)=>(b.authority||0)-(a.authority||0)||(b.released_at||0)-(a.released_at||0)||byName(a,b);
const releaseMonth=model=>{if(!model.released_at)return 0;const date=new Date(model.released_at*1000);return date.getUTCFullYear()*12+date.getUTCMonth()};
const byRecommended=(a,b)=>releaseMonth(b)-releaseMonth(a)||(b.authority||0)-(a.authority||0)||byRelease(a,b);
function sorted(models){return models.sort((a,b)=>state.sort==="recommended"?byRecommended(a,b):state.sort==="released"?byRelease(a,b):state.sort==="authority"?byAuthority(a,b):state.sort==="context"?b.context_length-a.context_length||byAuthority(a,b):state.sort==="output"?(b.max_output||0)-(a.max_output||0)||byAuthority(a,b):byName(a,b))}
...
function render(){
 ...
 if(state.provider){
  ...
  $("#groups").innerHTML=models.length?`<div class="grid model-stream">${models.map(card).join("")}</div>`:"";
 }
 updateURL();
}
function card(model){
 ...
 const release=model.released_at?new Intl.DateTimeFormat(state.lang==="zh"?"zh-CN":"en",{year:"numeric",month:"short",day:"numeric"}).format(new Date(model.released_at*1000)):label("unknown");
 return `<article class="card"><div class="card-title"><h4>${escapeHTML(state.lang==="zh"&&model.name_cn?model.name_cn:model.name)}</h4><button class="copy-id" type="button" data-copy="${escapeHTML(model.id)}" title="Copy model ID">⧉</button></div><div class="model-id"><strong>${escapeHTML(model.provider)}</strong> · ${escapeHTML(model.id)}</div><p class="description">${escapeHTML(description)}</p><div class="facts"><div class="fact"><span>${label("released")}</span><strong>${escapeHTML(release)}</strong></div><div class="fact"><span>${label("context")}</span><strong>${compact(model.context_length)}</strong></div><div class="fact"><span>${label("output")}</span><strong>${compact(model.max_output)}</strong></div></div><div class="tags">${model.tags.slice(0,8).map(tag=>`<span class="tag" data-tag="${tag}">${escapeHTML(tagLabel(tag))}</span>`).join("")}</div><div class="card-links">${linkMap.filter(([key])=>links[key]).slice(0,3).map(([key,text])=>`<a href="${escapeHTML(links[key])}" target="_blank" rel="noopener">${label(text)} ↗</a>`).join("")}</div></article>`
}
function updateURL(){const params=new URLSearchParams();if(state.query)params.set("q",state.query);if(state.provider)params.set("provider",state.provider);if(state.tags.size)params.set("tags",[...state.tags].join(","));if(state.sort!=="recommended")params.set("sort",state.sort);history.replaceState(null,"",params.size?`?${params}#catalog`:location.pathname+location.hash)}
function restoreURL(){const params=new URLSearchParams(location.search);state.query=params.get("q")||"";state.provider=params.get("provider")||"";state.sort=params.get("sort")||"recommended";state.tags=new Set((params.get("tags")||"").split(",").filter(Boolean));$("#search").value=state.query;$("#sort").value=state.sort}
...
$("#clear").addEventListener("click",()=>{state.query="";state.provider="";state.sort="recommended";state.tags.clear();$("#search").value="";$("#sort").value="recommended";renderProviderOptions();renderFilters();render()});
